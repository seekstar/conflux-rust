...
use cfxcore::{
    block_data_manager::DbType, pow::PowComputer,
    sync::utils::initialize_data_manager, vm::Env, vm_factory::VmFactory,
};
...
fn main() {
    ...
    let (data_man, _genesis_block) = initialize_data_manager(
        "./blockchain_data/blockchain_db",
        DbType::Rocksdb,
        pow,
        vm,
    );
    ...
    'epoch_loop: while let Some(epoch_later_hashes) =
        data_man.executed_epoch_set_hashes_from_db(epoch + 12)
    {
        ...
        for hash in hashes {
            ...
            let res = data_man
                ...
                .unwrap();
            let receipts = &res.1.block_receipts.as_ref().receipts;
            assert!(block.transactions.len() == receipts.len());
            let mut transactions = Vec::new();
            for (transaction, receipt) in
            ...
            block_trace.push(BlockTrace {
                ...
                env: Env {
                    ...
                    transaction_epoch_bound: conf
                        .raw_conf
                        .transaction_epoch_bound,
                },
            });
            let reward = match data_man.block_reward_result_by_hash_with_epoch(
                &hash,
                &epoch_later,
                false,
                false,
        ...
        let burnt_fee = data_man
            .db_manager
            .epoch_reward_result_from_db(&epoch_later)
            .unwrap()
            .burnt_fee;
